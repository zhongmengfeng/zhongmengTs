...
package org.tensorflow.demo.classifer;
...
public class TensorFlowObjectDetectionAPIModel implements Classifier {
  ...
  public List<Classifier.Recognition> recognizeImage(final Bitmap bitmap) {
    ...
    // on the provided parameters.
      ...
      int[] intValues = this.intValues;
      Log.e("recognizeImage",width+"        "+height);
      Log.e("recognizeImage",intValues.length+"     "+intValues.toString());
      bitmap.getPixels(this.intValues, 0, bitmap.getWidth(), 0, 0, bitmap.getWidth(), bitmap.getHeight());

    for (int i = 0; i < this.intValues.length; ++i) {
      byteValues[i * 3 + 2] = (byte) (this.intValues[i] & 0xFF);
      byteValues[i * 3 + 1] = (byte) ((this.intValues[i] >> 8) & 0xFF);
      byteValues[i * 3 + 0] = (byte) ((this.intValues[i] >> 16) & 0xFF);
    }
